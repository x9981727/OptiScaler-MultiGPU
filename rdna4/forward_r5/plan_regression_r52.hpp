...
#pragma once
// Included AFTER plan_core.hpp. Three reported command fragments only;
// this fixture is never accepted or executed as a complete GPU Forward.
namespace r52 {
inline r5::J boundary_fixture() {
    ...
    // Equivalent sparse final-argument fixture, explicitly indexed to avoid
    // errors when copying a 336-character run of mostly zero hex digits.
    // These are TEST fixture bytes; production always reads original bytes.
    std::string last(168*2,'0');
    last.replace(24*2,8,"80070000");
    last.replace(28*2,8,"80040000");
    last.replace(40*2,8,"20000000");
    last.replace(144*2,8,"01000000");
    ...
    return {{"fixture_scope","Three reported call fragments only; not the complete 154-launch trace"},
...
inline std::size_t regression_tests() {
    ...
    // Raw-plan hash validation remains outside Plan::parse. This small fixture
    // must also fail the complete 154-launch count contract.
    rejects(f,true);
