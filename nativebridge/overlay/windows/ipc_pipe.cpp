...
std::wstring name(DWORD pid,uint64_t instance){return L"\\\\.\\pipe\\MagpieNativeBridge-v2-"+std::to_wstring(pid)+L"-"+std::to_wstring(instance);}
bool tokenInfo(HANDLE token,TOKEN_INFORMATION_CLASS cls,std::vector<std::byte>& result){
    DWORD size=0;GetTokenInformation(token,cls,nullptr,0,&size);if(!size || size>65536)return false;
    result.resize(size);return GetTokenInformation(token,cls,result.data(),size,&size)!=FALSE;
}
HRESULT finish(HANDLE pipe,OVERLAPPED& ov,BOOL immediate,DWORD immediateError,DWORD timeout,DWORD& count){
    ...
    CancelIoEx(pipe,&ov);DWORD drained=0;
    if(GetOverlappedResult(pipe,&ov,&drained,TRUE)){count=drained;return S_OK;}
    const DWORD completionError=GetLastError();
    if(wait==WAIT_TIMEOUT && completionError==ERROR_OPERATION_ABORTED)return HRESULT_FROM_WIN32(ERROR_TIMEOUT);
...
void Channel::Close() noexcept{
    ...
    if(pipe_!=INVALID_HANDLE_VALUE){if(listener_)DisconnectNamedPipe(pipe_);CloseHandle(pipe_);pipe_=INVALID_HANDLE_VALUE;}
...
HRESULT Channel::Listen(uint64_t instance) noexcept{
    ...
    try{
        ...
        std::vector<std::byte> user;if(!tokenInfo(token.h,TokenUser,user))return E_ACCESSDENIED;
        LPWSTR sid=nullptr;if(!ConvertSidToStringSidW(reinterpret_cast<TOKEN_USER*>(user.data())->User.Sid,&sid))return last();
        Local sidOwner{sid};const std::wstring sddl=L"D:P(A;;GA;;;"+std::wstring(sid)+L")";
        PSECURITY_DESCRIPTOR descriptor=nullptr;
        if(!ConvertStringSecurityDescriptorToSecurityDescriptorW(sddl.c_str(),SDDL_REVISION_1,&descriptor,nullptr))return last();
        Local descriptorOwner{descriptor};SECURITY_ATTRIBUTES security{sizeof(SECURITY_ATTRIBUTES),descriptor,FALSE};
        ...
        if(pipe_==INVALID_HANDLE_VALUE)return last();listener_=true;return S_OK;
    }catch(...){Close();return E_OUTOFMEMORY;}
}
HRESULT Channel::Authenticate(bool server,DWORD expected) noexcept{
    ULONG pid=0;if(!(server?GetNamedPipeClientProcessId(pipe_,&pid):GetNamedPipeServerProcessId(pipe_,&pid)))return last();
    if(!pid || (expected && pid!=expected))return E_ACCESSDENIED;
    Handle process{OpenProcess(PROCESS_QUERY_LIMITED_INFORMATION|PROCESS_DUP_HANDLE,FALSE,pid)};if(!process.h)return last();
    Handle selfToken,otherToken;
    if(!OpenProcessToken(GetCurrentProcess(),TOKEN_QUERY,&selfToken.h) || !OpenProcessToken(process.h,TOKEN_QUERY,&otherToken.h))return last();
    try{
        std::vector<std::byte> a,b;if(!tokenInfo(selfToken.h,TokenUser,a)||!tokenInfo(otherToken.h,TokenUser,b))return E_ACCESSDENIED;
        if(!EqualSid(reinterpret_cast<TOKEN_USER*>(a.data())->User.Sid,reinterpret_cast<TOKEN_USER*>(b.data())->User.Sid))return E_ACCESSDENIED;
        ...
        if(sa.AuthenticationId.LowPart!=sb.AuthenticationId.LowPart||sa.AuthenticationId.HighPart!=sb.AuthenticationId.HighPart||sessionA!=sessionB)return E_ACCESSDENIED;
...
HRESULT Channel::Accept(DWORD timeout,DWORD expected) noexcept{
    ...
    // A cancelled pending ConnectNamedPipe leaves this pipe instance available
    // for another ConnectNamedPipe call. Keep the listener alive so Magpie may
    // start after the game without forcing NativeBridge reinitialization.
    if(FAILED(hr) && hr!=timeoutHr)Close();
    return hr;
}
HRESULT Channel::Connect(DWORD pid,uint64_t instance,DWORD timeout) noexcept{
    ...
    try{
        ...
        while(true){
            ...
            if(pipe_!=INVALID_HANDLE_VALUE)break;const DWORD error=GetLastError();
            if(error!=ERROR_PIPE_BUSY && error!=ERROR_FILE_NOT_FOUND)return HRESULT_FROM_WIN32(error);
            if(GetTickCount64()-start>=timeout)return HRESULT_FROM_WIN32(ERROR_TIMEOUT);Sleep(1);
        }
...
HRESULT Channel::Transfer(bool write,std::span<std::byte> data,DWORD timeout) noexcept{
    ...
    Handle event{CreateEventW(nullptr,TRUE,FALSE,nullptr)};if(!event.h)return last();
    OVERLAPPED ov{};ov.hEvent=event.h;DWORD count=0;
    const BOOL ok=write?WriteFile(pipe_,data.data(),DWORD(data.size()),nullptr,&ov):ReadFile(pipe_,data.data(),DWORD(data.size()),nullptr,&ov);
    const DWORD error=ok?ERROR_SUCCESS:GetLastError();HRESULT hr=finish(pipe_,ov,ok,error,timeout,count);
    if(SUCCEEDED(hr)&&count!=data.size())hr=HRESULT_FROM_WIN32(ERROR_INVALID_DATA);
    const HRESULT timeoutHr=HRESULT_FROM_WIN32(ERROR_TIMEOUT);
    if(FAILED(hr) && (write || hr!=timeoutHr))Close();return hr;
}
HRESULT Channel::Send(const Message& msg,DWORD timeout) noexcept{
    const auto kind=static_cast<uint32_t>(msg.kind);if(kind<1||kind>5||msg.length>PayloadCapacity)return E_INVALIDARG;
    std::array<std::byte,Bytes> bytes{};u32(bytes.data(),Magic);u32(bytes.data()+4,kind);u64(bytes.data()+8,msg.sequence);
    u32(bytes.data()+16,msg.slot);u32(bytes.data()+20,msg.length);std::copy_n(msg.payload.begin(),msg.length,bytes.begin()+Header);
    u32(bytes.data()+Bytes-4,wire::crc32(std::span(bytes).first(Bytes-4)));return Transfer(true,bytes,timeout);
}
HRESULT Channel::Receive(Message& output,DWORD timeout) noexcept{
    ...
    Message msg{};msg.kind=static_cast<Kind>(type);msg.sequence=r64(bytes.data()+8);msg.slot=r32(bytes.data()+16);msg.length=length;
...
HRESULT Channel::DuplicateToPeer(HANDLE source,HANDLE& output)const noexcept{
    ...
    if(!DuplicateHandle(GetCurrentProcess(),source,peer_,&output,0,FALSE,DUPLICATE_SAME_ACCESS))return last();return S_OK;
}
HRESULT Channel::RevokeUnsent(HANDLE handle)const noexcept{
    if(!Connected()||!handle||handle==INVALID_HANDLE_VALUE)return E_INVALIDARG;HANDLE local=nullptr;
